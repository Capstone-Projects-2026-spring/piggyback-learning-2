...
#### 1. User Hierarchy and Permissions
The system uses a Class Inheritance model for users:
User (Base Entity): Has the core credentials (username/password hash) and identification for any person logging into the app.
Parent: Inherits from User. A Parent acts as the admin, capable of managing multiple Kid accounts and viewing their performance data.
Kid: Inherits from User. A Kid account is attached to a Parent ID and represents the learner. Kids interacts with assigned videos and receive recommendations based on their Tags.

#### 2. Video Content and Structure
Videos are the main focus of the app. The system breaks a video down into smaller parts for the AI to analyze:
Video: Represents a YouTube-sourced file, containing metadata like the YouTube ID, local storage paths, and duration.
Segment: A video is divided into specific time-stamped "Segments" (ex. 0:30 to 1:00). These define the quiz intervals.
Question: Each Segment contains multiple questions generated by the AI.
Frame: During processing, FFmpeg extracts image frames from the video. These frames are stored and linked to the Video ID so the AI can see the content it is writing questions about.

#### 3. Learning Progress and Data
To track how a child is performing, the system uses a assignment and answer model:
VideoAssignment: This connects a Kid and a Video. It tracks which videos a child is allowed to watch.
Answer (Struct): This is a data structure stored as a JSON list within the VideoAssignment. It captures the Data from every quiz attempt:
Transcript: What the child said (via Vosk).
Similarity Score: How close the answer was to the expected answer.
Mood/Energy: The mood detected by the audio analysis.

#### 4. System Controllers and Managers
These classes are part of the backend, where logic is executed:
AuthController: Manages security (signups, logins, and JWT token issuance).
VideoController: Handles tasks like downloading videos via yt-dlp and extracting frames via FFmpeg.
AnswerController: The grading program. It triggers the speech-to-text transcription and performs the similarity/mood analysis.
WS_Manager: The WebSocket coordinator. It keeps a map of active users and broadcasts real time updates (ex. Download 50% complete) to the frontend.

#### 5. Personalization with Tags
The Tag system creates a Many-to-Many relationship.
Parents assign Tags (ex. "Spiderman" "Skibidi Toilet") to a Kid.
Videos are also assigned Tags.
The system compares these lists to provide a Recommendation feed tailored to the child's interests.

---

**Backend**
#### Built with Loco.rs, which integrates:
- Axum – Used for the Web server and routing
...
- Role Access Control: Implements a system to log in using Parent and Kid accounts, which have separate roles.
...
- JWT Security: Uses JSON Web Tokens (JWT) for management of secure sessions, with secrets managed via environment variables.

#### Video Processing
- YouTube Downloading: Uses yt-dlp to fetch videos, metadata, and subtitles from a YouTube URL.
- Frame Extraction: Uses FFmpeg to extract frames from downloaded videos for AI to use during question generation.
- Processing is done with API endpoints, and the progress of processing is streamed to the frontend over WebSockets.
...
Generated assets (the videos, extracted frames, and question JSON files) are served statically and can be accessed by URL for the frontend to use.
...
- Pages are composed of reusable components located in components/.
...
- Record audio responses and transcribe them with backend APIs.
...
- Attention Logic: If the application detects the child has looked away for a period of time, an automatic video pause is triggered, and the app sends a distraction notification to the parent through the backend.

### Static/Media Serving:
- Static assets are reachable from the public/ directory (Next.js convention).
- videos, extracted frames, and question JSON can be reached by the backend and accessed via API routes.
...
- That React site is for docs only and has its own build/runtime flow.
